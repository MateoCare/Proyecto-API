...
public interface ListaProductosRepository extends PagingAndSortingRepository<Producto, Long> {
    ...
    @Query(value = "select p.*, count(1) cont from producto p inner join favorito f on p.id = f.producto_id group by p.id order by cont desc LIMIT 10", nativeQuery = true)
